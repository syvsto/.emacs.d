...
Helm-bufler lets helm use bufler as a completion source.
#+begin_src emacs-lisp
  (use-package helm-bufler
    :commands (helm-bufler-switch-buffer)
    :bind ("C-x b" . my/helm-bufler-mini))
#+end_src

It's nice to have a helm source for that includes bufler buffers together with
recent files etc., akin to ~helm-mini~.

#+begin_src emacs-lisp
  (defun my/helm-bufler-mini ()
    (interactive)
    (helm :sources (list helm-bufler-source helm-source-recentf helm-source-buffer-not-found)))
#+end_src
...
#+begin_src emacs-lisp
  (defun my/bind-helm-lsp ()
    ...
    (define-key lsp-mode-map [remap lsp-execute-code-action] #'helm-lsp-code-action))
