...
#+begin_src emacs-lisp
  ...
  (use-package evil-collection
    :after evil
    :custom (evil-want-integration nil)
    :config
...
#+begin_src emacs-lisp
  (use-package general
    ...
    (defhydra hydra-leader (:exit t :hint nil)
      "
  ^Modes^                                                 ^Applications^ ^^^^^^^^--------------------------------------------------------------------------
      _q_: activity _w_: line  _f_: word   _p_: char         _._: Eshell
      _a_: window   _r_: file  _s_: buffer _t_: view         _u_: Undo tree
      ^ ^           _x_: symex ^ ^         ^ ^               _m_: Magit
      "
...
#+BEGIN_SRC emacs-lisp
  ...
  (set-face-attribute 'fixed-pitch nil :family "JuliaMono" :height 110)
  (set-face-attribute 'variable-pitch nil :family "JuliaMono" :height 110)
#+END_SRC
