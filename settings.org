...
#+begin_src emacs-lisp
  ...
  (defun my/embark-live-occur-fit-window (&rest _)
  ...
  (use-package embark
    ...
    :hook ((minibuffer-setup . embark-live-occur-after-input)
	   (embark-occur-post-revert . my/embark-live-occur-fit-window))
    :bind
    ("C-," . embark-act)
...
#+begin_src emacs-lisp
  (use-package avy-embark-occur
    :straight (avy-embark-occur :type git :host github :repo "oantolin/embark" :file "avy-embark-occur.el")
    :bind (:map minibuffer-local-completion-map
...
small ones), I don't always have the letters /æ/, /ø/ and /å/
available. With the following function, I can use the /ae/, /oe/ and /aa/
spellings instead, which are converted to the correct letter.
...
Aggressive-indent keeps a code buffer indented at all times. This is very nice
for programming in languages that don't have standard code formatters, such as
lisp.
