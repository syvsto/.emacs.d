...
I don't like popups when coding, so I trigger autocomplete manually. Since I
mostly use the various LSP servers for code, ~capf~ can be used for completion
through ~completion-at-point~.
