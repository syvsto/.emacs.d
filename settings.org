...
Company mode is what I'm used to. Enable ~capf~ completion, since that's
what ~lsp-mode~ uses.
...
~company-box~ adds some nice icons to company-mode. 
...
The default colors for company popups are a bit jarring, so use
something else.
...
#+begin_src emacs-lisp
  (use-package lsp-ivy
    ...
    :config
    (define-key lsp-mode-map [remap lsp-ui-peek-find-workspace-symbol] #'lsp-ivy-workspace-symbol))
#+end_src
...
etc. commands. Bind this to a leader binding.

#+begin_src emacs-lisp :noweb-ref "leader-rust" :tangle no
  (meow-leader-define-key
   'rustic-mode
   '("c" . rustic-popup))
#+end_src
