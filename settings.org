...
Since Emacs 27, the built-in IComplete does the job of completion well. By
adding a more flexible candidate selection through ~orderless~, it becomes even
better. Orderless is a thin layer on top of the default ~completion-at-point~, so
it works in ~company~ as well. I'm too used to vertical selection by now, so the
~icomplete-vertical~ package helps with that
